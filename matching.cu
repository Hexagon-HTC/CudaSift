...
double MatchSiftData(SiftData &data1, SiftData &data2)
{
    ...
    if (data1.h_data != NULL)
    {
        ...
        // here we are coping the block of SiftPoint starting at score ending on match_ypos (4 floats + 1 int)
        safeCall(cudaMemcpy2D(h_ptr, sizeof(SiftPoint), d_ptr, sizeof(SiftPoint), 4 * sizeof(float) + sizeof(int), data1.numPts, cudaMemcpyDeviceToHost));
    }
